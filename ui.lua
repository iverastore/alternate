            ...
            pcall(function()
                if library.notifications and library.notifications.create_notification then
                    ...
                    local notification = library:create("Frame", {
                        Parent = library.gui or coregui,
                        Size = dim2(0, 250, 0, 60),
                        Position = dim2(1, -270, 0, 20),
                    ...
                    -- Animate in
                    notification:TweenPosition(dim2(1, -270, 0, 20), "Out", "Quad", 0.3)
                    
                    -- Auto remove
                    task.delay(duration, function()
                        if notification and notification.Parent then
                            notification:TweenPosition(dim2(1, 0, 0, 20), "In", "Quad", 0.3)
    ...
    -- Library element functions
        function library:window(properties)
            ...
            -- Built-in Settings tab
                local settingsTab = library:tab({name = "Settings"})
                local settingsCol1 = settingsTab:column({fill = true})
                local settingsCol2 = settingsTab:column({fill = true})

                -- Config directory setup
                local configDir = "alternate.lol/configs"
                ...
                -- Configs section
                local configsSection = settingsCol1:section({name = "Configs"})
                local configNameFlag = ""
                local cfgDropdown = configsSection:addDropdown({ 
                    name = "Config List", 
                    flag = "CfgSelect", 
                    items = getConfigList(), 
                    default = nil 
                ...
                configsSection:addTextBox({ 
                    ...
                    callback = function(v) configNameFlag = v end 
                })

                configsSection:addButton({ 
                    name = "Save Config", 
                    callback = function()
                        local name = library.flags["CfgName"] or configNameFlag
                        if name == "" then 
                            if library.Notify then library:Notify("Please enter a config name!", 3) end
                            return 
                        end
                        ...
                        for flag, value in pairs(library.flags) do
                            ...
                            if flag:match("^Cfg") then continue end
                            ...
                            elseif t == "Color3" then
                                data[flag] = {_type="Color3", R=math.floor(value.R*255+0.5), G=math.floor(value.G*255+0.5), B=math.floor(value.B*255+0.5)}
                            elseif t == "table" then
                                if value.Color and typeof(value.Color) == "Color3" then
                                    data[flag] = {
                                        _type = "ColorPicker",
                                        Color = {_type="Color3", R=math.floor(value.Color.R*255+0.5), G=math.floor(value.Color.G*255+0.5), B=math.floor(value.Color.B*255+0.5)},
                                ...
                                elseif value.key or value.Key then
                                    ...
                                    data[flag] = {
                                        ...
                                        key = keyVal and tostring(keyVal) or "NONE",
                        ...
                        pcall(function()
                            ...
                            if library.Notify then library:Notify("Config saved: " .. name, 3) end
                        end)
                        
                        pcall(function() 
                            if cfgDropdown and cfgDropdown.refresh_options then 
                                cfgDropdown.refresh_options(getConfigList()) 
                            end 
                        end)
                    end 
                })
...
return library, notifications, themes
                configsSection:addButton({ 
                    name = "Load Config", 
                    callback = function()
                        local name = library.flags["CfgSelect"]
                            ...
                            if ok and raw then
                                ...
                                if success and data then
                                    for flag, value in pairs(data) do
                                        ...
                                        if library.config_flags and library.config_flags[flag] then
                                            pcall(function() library.config_flags[flag](value) end)
                                        end
                                    end
                                    if library.Notify then library:Notify("Config loaded: " .. name, 3) end
                                else
                                    if library.Notify then library:Notify("Failed to parse config file", 3) end
                                end
                            else
                                if library.Notify then library:Notify("Failed to read config file", 3) end
                            end
                        else
                            if library.Notify then library:Notify("Please select a config to load", 3) end
                        end
                    end 
                })
                ...
                configsSection:addButton({ 
                    ...
                    callback = function()
                        ...
                        if name and name ~= "" then
                            pcall(function() 
                                delfile(configDir .. "/" .. name .. ".cfg") 
                                if library.Notify then library:Notify("Config deleted: " .. name, 3) end
                            end)
                            pcall(function() 
                                if cfgDropdown and cfgDropdown.refresh_options then 
                                    cfgDropdown.refresh_options(getConfigList()) 
                            ...
                            if library.Notify then library:Notify("Please select a config to delete", 3) end
                        end
                    end 
                })
                ...
                configsSection:addButton({ 
                    ...
                    callback = function()
                        ...
                        if library.Notify then library:Notify("Config list refreshed", 2) end
                    end 
                })

                -- Menu section
                local menuSection = settingsCol1:section({name = "Menu"})
                    ...
                    callback = function(v)
                        pcall(function()
                            if library.TargetHUDObj and library.TargetHUDObj.SetVisibility then 
                                library.TargetHUDObj:SetVisibility(v) 
                            end
                        end)
                    end 
                })
                ...
                menuSection:addSlider({ 
                    ...
                    callback = function(v)
                        pcall(function()
                            if library.set_element_scale then 
                                library:set_element_scale(v) 
                            end
                        end)
                    end 
                })
                -- Notifications section
                local notifSection = settingsCol2:section({name = "Notifications"})
                
                notifSection:addToggle({ 
                ...
                notifSection:addSlider({ 
                    ...
                    suffix = "s", 
                    callback = function(v)
                        if not library.NotifSettings then library.NotifSettings = {} end
                        library.NotifSettings.Duration = v
                    end 
                })

                notifSection:addDropdown({ 
                    name = "Position", 
                    flag = "NotifPosition", 
                    items = {
                ...
                notifSection:addButton({ 
                    ...
                    callback = function()
                        local duration = library.NotifSettings and library.NotifSettings.Duration or 3
                        if library.Notify then 
                            library:Notify("This is a test notification!", duration) 
                        end
                    end 
                })

                -- Themes section
                local themeSection = settingsCol2:section({name = "Theme Colors"})
                
                themeSection:addLabel({name = "Primary Colors"})
                ...
                themeSection:addColorPicker({ 
                    ...
                    callback = function(c)
                        if typeof(c) == "Color3" then 
                            pcall(function() 
                                library:updateTheme("background", c) 
                            end) 
                        end
                    end 
                })
                ...
                themeSection:addColorPicker({ 
                    ...
                    callback = function(c)
                        if typeof(c) == "Color3" then 
                            pcall(function() 
                                library:updateTheme("element", c) 
                            end) 
                        end
                    end 
                })

                themeSection:addLabel({name = "Secondary Colors"})
                ...
                themeSection:addColorPicker({ 
                    ...
                    callback = function(c)
                        if typeof(c) == "Color3" then 
                            pcall(function() 
                                library:updateTheme("element2", c) 
                            end) 
                        end
                    end 
                })
                ...
                themeSection:addColorPicker({ 
                    name = "Unselected", 
                    flag = "ThemeUnselected", 
                    color = Color3.fromRGB(120,120,120), 
                    callback = function(c)
                        if typeof(c) == "Color3" then 
                            pcall(function() 
                                library:updateTheme("unselected", c) 
                            end) 
                        end
                    end 
                })
                ...
                -- Performance section
                local perfSection = settingsCol2:section({name = "Performance"})
                ...
                -- Information section
                local infoSection = settingsCol1:section({name = "Information"})
                
                infoSection:addLabel({name = "Library: alternate.lol"})
                infoSection:addLabel({name = "Version: " .. (library.version or "1.0.0")})
                infoSection:addLabel({name = "Game: " .. (game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name or "Unknown")})
